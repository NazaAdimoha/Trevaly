    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar='sidebar'
            className='bg-sidebar group-data-[variant=floating]:border-grey-50 flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow'
          >
            {children}
          </div>
        </div>
...
const sidebarMenuButtonVariants = cva(
  [
    ...
    // Transition
    'transition-[width,height,padding] ease-in-out duration-200',
    // Hover state
    ...
    '[&>span:last-child]:whitespace-normal [&>svg]:size-5 [&>svg]:shrink-0',
  ],
  {
    variants: {
      variant: {
      ...
      size: {
        ...
        sm: 'min-h-7 text-xs',
        lg: 'min-h-12 text-base group-data-[collapsible=icon]:!p-0',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Varied skeleton width (50-90%), derived from a stable per-instance id
  // rather than Math.random(), which is impure during render.
