...
async function main() {
  ...
  // The public key belongs to the web app (the checkout popup); the secret key
  // to this API. Checking that their modes match needs both.
  const webEnv = '../custom_ecommerce_build/.env';
  const publicKey =
    process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY ??
    (existsSync(webEnv) ? parse(readFileSync(webEnv)).NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY : undefined);
